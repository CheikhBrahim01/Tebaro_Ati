// src/pages/AdminTransactionWithdrawalPage.jsx
import React, { useState, useEffect } from 'react';
import axios from '../api/api';
import authService from '../services/authService';

const AdminTransactionWithdrawalPage = () => {
  ...
  const fetchTransactions = async () => {
    try {
      setLoading(true);
      const res = await axios.get('/donations/admin/transactions/withdrawal/', {
        headers: {
      ...
      setError('Failed to fetch withdrawal transactions.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="p-8 bg-gray-50 min-h-screen">
      <h1 className="text-3xl font-bold text-center mb-6">Withdrawal Transactions</h1>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50">
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 z-10 max-w-lg w-full">
            ...
            <div className="space-y-2">
              ...
              <p><strong>Type:</strong> {selectedTx.transaction_type}</p>
              <p><strong>Amount:</strong> ${selectedTx.amount}</p>
              <p><strong>Transaction Number:</strong> {selectedTx.transaction_num}</p>
              <p><strong>Phone Number:</strong> {selectedTx.phone_number}</p>
              ...
              {selectedTx.photo && (
                <div className="mt-2">
                  ...
                  <img src={selectedTx.photo} alt="Transaction Screenshot" className="mt-2 max-h-60 object-contain" />
                </div>
              )}
            </div>
            <div className="mt-6 flex justify-end space-x-4">
              <button onClick={() => updateStatus('approved')} className="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded">
                Validate
              </button>
              <button onClick={() => updateStatus('rejected')} className="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded">
                Not Valid
              </button>
...
export default AdminTransactionWithdrawalPage;
